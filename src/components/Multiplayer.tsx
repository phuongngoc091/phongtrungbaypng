...
export const Multiplayer = ({ galleryId }: { galleryId: string }) => {
  ...
  // Always append a random string so same-account multiple tabs don't overwrite each other
  const [myId] = useState(() => {
    const baseId = profile?.uid || 'guest'
    return `${baseId}_${Math.random().toString(36).substring(7)}`
  })
